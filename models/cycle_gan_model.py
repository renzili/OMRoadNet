...
class CycleGANModel(BaseModel):
    ...
    def forward(self):
        ...
        self.fake_B, mu1 = self.netG_A(self.real_A)  # G_A(A)
        with torch.no_grad():
        ...
        self.rec_A, mu2 = self.netG_B(self.fake_B)   # G_B(G_A(A))
        with torch.no_grad():
        ...
        self.fake_A, mu3 = self.netG_B(self.real_B)  # G_B(B)
        with torch.no_grad():
        ...
        self.rec_B, mu4 = self.netG_A(self.fake_A)   # G_A(G_B(B))
        with torch.no_grad():
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A, mu5 = self.netG_A(self.real_B)
            with torch.no_grad():
            ...
            self.idt_B, mu6 = self.netG_B(self.real_A)
            with torch.no_grad():
