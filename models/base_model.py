...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.save_transfermodel = os.path.join(opt.checkpoints_dir, "transfer_learning")
    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            ...
            self.load_networks_transferlearn(load_suffix)  # 加载迁移学习的模型
    ...
    def update_learning_rate(self):
        ...
        # print(self.optimizers[1].param_groups[0]['lr'])
    ...
    def load_networks_transferlearn(self, epoch):
        """Load all the networks from the disk.
        ...
        for name in self.model_names:
            if isinstance(name, str):
                load_filename = '%s_net_%s.pth' % (epoch, name)
                load_path = os.path.join(self.save_transfermodel, load_filename)
                net = getattr(self, 'net' + name)
                if isinstance(net, torch.nn.DataParallel):
                    net = net.module
                print('loading the model from %s' % load_path)
