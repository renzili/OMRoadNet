...
def transform_invert(img):
    # Tensor -> PIL.Image
    # 注意：img.shape = [3, 2160, 3840]

    if img.dim() == 3:  # single image # 3, 2160, 3840
        img = img.unsqueeze(0)  # 在第0维增加一个维度 1,3, 2160, 3840
    low = float(img.min())
    high = float(img.max())
...
def test_dt():
    ...
    transform_invert(value['A'])
    transform_invert(value['B'])
